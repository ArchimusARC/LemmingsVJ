...
void SceneLevel2::init()
{
	...
	glm::vec2 texCoords[2] = { glm::vec2(float(displ) / 1024.0f, 0.f), glm::vec2((float(displ) + 320.0f) / 1024.0f, 160.f / 256.0f) };
	...
	maskTexture.loadFromFile("images/level2_mask2.png", TEXTURE_PIXEL_FORMAT_L);
	...
	goal.init(glm::vec2(767, 110), simpleTexProgram, displ);
	lemmingsInitiated = 0;
	...
	++lemmingsInitiated;
	toolbar.init(glm::vec2(0, 144), simpleTexProgram, displ);
	accel = false;
	pause = false;
